...
lattice_listB <- list('marine/networks_old/Lattice_knn_k3.graph', 'marine/networks_old/Lattice_knn_k8.graph',
                      'marine/networks_old/Lattice_knn_k3_FST.graph', 'marine/networks_old/Lattice_knn_k8_NE.graph')
lattice_listD <- list('marine/networks_old/Lattice_knn_k1.graph', 'marine/networks_old/Lattice_knn_k7.graph',
                      'marine/networks_old/Lattice_knn_k1_FST.graph', 'marine/networks_old/Lattice_knn_k8_NE.graph')
...
binmod <- Map(function(dat, r, lati) run_INLA_model(data = dat, predictor = 'PA_bin', response = r, Lattice.adj = lati),
              ...
              r = response_all, 
              lati = lattice_listB)
#saveRDS(binmod, 'marine/model_output/bin_models_marine.rds')
...
distmod <- Map(function(dat, r, lati) run_INLA_model(data = dat, predictor = 'scale_PA_dist', response = r, Lattice.adj = lati),
               ...
               r = response_all, 
               lati = lattice_listD)
#saveRDS(distmod, 'marine/model_output/dist_models_marine.rds')
...
mfixef_both <- rbind(fixef_dfb, fixef_df)
mfrdat_both <- rbind(frdatb, frdat)

# Reorder factor levels for plot
mfixef_both$response_var <- factor(mfixef_both$response_var, levels = var_list[c(3,2,1,4)])

mfrdat_both$response_var <- factor(mfrdat_both$response_var, levels = var_list[c(3,2,1,4)])
