...
# Data -------------
grdat_env <- read.csv('data/genetdata_PA_mar', h=T) 
...
for (i in 1:10) {
  ...
  nb2INLA(paste0("networks/marine/Lattice_knn_k", i, ".graph"), nb2knn)
  
  #FST
  nb2knn_FST = chooseCN(xyfst, type = 6, result.type = "nb", k = i, edit.nb = F)
  nb2INLA(paste0("networks/marine/Lattice_knn_k", i, "_FST.graph"), nb2knn_FST)
  
  #NE
  nb2knn_NE = chooseCN(xygd_ne, type = 6, result.type = "nb", k = i, edit.nb = F)
  nb2INLA(paste0("networks/marine/Lattice_knn_k", i, "_NE.graph"), nb2knn_NE)
}
...
#loop over different adjacency matrices
for (i in 1:10) {
  Lattice.adj = paste0("networks/marine/Lattice_knn_k", i, ".graph")
  LatticeFST.adj = paste0("networks/marine/Lattice_knn_k", i, "_FST.graph")
  LatticeNE.adj = paste0("networks/marine/Lattice_knn_k", i, "_NE.graph")
...
write.csv(DIC_result, file = 'DIC_connectivity_matrix_PA_besag_mar.csv')

colnames(WAIC_result) = col_nam
rownames(WAIC_result) = row_nam
write.csv(WAIC_result, file = 'WAIC_connectivity_matrix_PA_besag_mar.csv')

colnames(Moran_I_result) = col_nam
rownames(Moran_I_result) = row_nam
write.csv(Moran_I_result, file = 'MoranI_conn_matrix_PA_besag_mar.csv')

colnames(Moran_p_result) = col_nam
rownames(Moran_p_result) = row_nam
write.csv(Moran_p_result, file = 'Moranp_conn_matrix_PA_besag_mar.csv')
...
write.csv(sf_res, 'model_coef_compare_k_mar.csv', quote = F)
...
colnames(WAIC_result) = col_nam
rownames(WAIC_result) = row_nam
write.csv(WAIC_result, file = 'WAIC_connectivity_matrix_PA_DIST_besag_mar.csv')

colnames(Moran_I_result) = col_nam
rownames(Moran_I_result) = row_nam
write.csv(Moran_I_result, file = 'MoranI_conn_matrix_PA_DIST_besag_mar.csv')

colnames(Moran_p_result) = col_nam
rownames(Moran_p_result) = row_nam
write.csv(Moran_p_result, file = 'Moranp_conn_matrix_PA_DIST_besag_mar.csv')
...
sf_res <- rbind(sf_gdr, sf_arr, sf_arrsp, sf_fstr, sf_ner)
write.csv(sf_res, 'model_coef_compare_k_DIST_besag_mar.csv', quote = F)
